...
class LogParser(object):
    ...
    def __init__(self, config_file):
        ...
        json_formatter = jsonlogger.JsonFormatter('%(asctime)%(levelname)%(message)')
        # console logging
        ...
        # devel.log file
        devel_log = logging.handlers.TimedRotatingFileHandler(filename='devel.log', when='midnight', encoding='utf8')
        devel_log.setLevel(logging.DEBUG)
        devel_log.setFormatter(json_formatter)
    ...
    def parse_line(self, string):
        ...
        option = {'InitGame': self.new_game, 'Warmup': self.handle_warmup, 'InitRound': self.handle_initround,
                  ...
                  'Flag': self.handle_flag, 'FlagCaptureTime': self.handle_flagcapturetime, 'Hotpotato': self.handle_hotpotato,
    ...
    def new_game(self, line):
        ...
        map_name = values['mapname'] if 'mapname' in values else "UnknownMap"
        g_gametype = values['g_gametype'] if 'g_gametype' in values else "GameTypeNotFound"

        logger.debug("InitGame: Starting game...",
            extra = {'event_type': 'init_game', 'event_properties': {
            'map_name': map_name,
    ...
    def handle_spawn(self, line):
        ...
        """
        player_num = int(line)
        logger.debug("Player %d spawned", player_num
            , extra = {'event_type': 'player_spawn', 'event_properties': {
            'player': self.game.players[player_num].get_guid()
        }})
        with self.players_lock:
    ...
    def handle_flagcapturetime(self, line):
        ...
        if action.isdigit():
            ...
            logger.debug("Player %d captured the flag in %s seconds", player_num, cap_time
                , extra = {'event_type': 'flag_capture_time', 'event_properties': {
                'player': self.game.players[player_num].get_guid(),
                'capture_time_ms': action
            }})
            with self.players_lock:
    ...
    def handle_warmup(self, line):
        ...
        os.system('Rscript /home/daniel/ut-eterstats.github.io/deploy.R')
        self.allow_cmd_teams = True


    def handle_hotpotato(self, _):
        """
        handle Hot Potato
        """
        logger.debug("hotpotato")
    ...
    def handle_disconnect(self, line):
        ...
        with self.players_lock:
            ...
            logger.debug("ClientDisconnect: Player %d %s has left the game", player_num, player.get_name()
                , extra = {'event_type': 'player_disconnected', 'event_properties': {
                'player': self.game.players[player_num].get_guid()
            }})
    ...
    def handle_hit(self, line):
        ...
        with self.players_lock:
            ...
            if hitpoint in self.hit_points:
                ...
                logger.debug("Player %d %s hit %d %s in the %s with %s",
                    hitter_id, hitter_name, victim_id, self.game.players[victim_id].get_name(), self.hit_points[hitpoint], self.hit_item[hit_item]
                    , extra = {'event_type': 'hit', 'event_properties': {
                        'hitter': hitter.get_guid(),
                        'victim': self.game.players[victim_id].get_guid(),
                        'hit_point': self.hit_points[hitpoint],
                        'hit_item': self.hit_item[hit_item]
                    }})
        ...
        with self.players_lock:
            ...
            if not self.ffa_lms_gametype:
                if (victim.get_team() == killer.get_team() and victim_id != killer_id) and death_cause != "UT_MOD_BOMBED":
                    ...
                    , extra = {'event_type': 'kill', 'event_properties': {
                        'killer': killer.get_guid(),
                        'victim': victim.get_guid(),
            ...
            if death_cause in suicide_reason or (killer_id == victim_id and death_cause in suicide_weapon):
                ...
                , extra = {'event_type': 'suicide', 'event_properties': {
                    'victim': victim.get_guid(),
                    'death_cause': death_cause
                }})
            # kill counter
    ...
    def handle_flag(self, line):
        ...
        with self.players_lock:
            ...
            if action == '1:':
                ...
                , extra = {'event_type': 'flag_returned', 'event_properties': {
                    'player': player.get_guid()
                }})
            elif action == '2:':
                player.capture_flag()
                ...
                , extra = {'event_type': 'flag_captured', 'event_properties': {
                    'player': player.get_guid()
                }})
            elif action == '0:':
                logger.debug("Player %d dropped the flag", player_num
                , extra = {'event_type': 'flag_dropped', 'event_properties': {
                    'player': player.get_guid()
                }})
...
#curs.execute('CREATE TABLE IF NOT EXISTS daily_stats (id INTEGER PRIMARY KEY NOT NULL, game_date DATE NOT NULL, guid TEXT NOT NULL, name TEXT NOT NULL, kills INTEGER DEFAULT 0, deaths INTEGER DEFAULT 0, headshots INTEGER DEFAULT 0, team_kills INTEGER DEFAULT 0, team_death INTEGER DEFAULT 0, max_kill_streak INTEGER DEFAULT 0, suicides INTEGER DEFAULT 0, ratio REAL DEFAULT 0, rounds INTEGER DEFAULT 0, admin_role INTEGER DEFAULT 1)')
